  ...
  return (
    <div ref={rootRef} className="relative" data-guide="lang">
      ...
      <button
        ...
        aria-expanded={open}
      >
        {showIcon && <Icon name="globe" className="w-4 h-4 opacity-70" />}
        <span className="text-sm">{current.label}</span>
        {showIcon && <Icon name="chevronDown" className={`w-3.5 h-3.5 opacity-70 transition-transform ${open ? 'rotate-180' : ''}`} />}
      </button>
